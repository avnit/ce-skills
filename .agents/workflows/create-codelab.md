...
6. **Phase 3.5: Pre-Flight Code Audit Gate (Mandatory Pre-Flight Audit)**
   - Execute the pre-flight code audit script on the generated narrative file before provisioning any cloud resources:
   ...
   - **Pre-Flight Audit Gate**: On `FAIL` (due to syntax errors, unresolved `<...>` placeholders, or invalid `gcloud` command groups), auto-remediate the `.lab.md` narrative — consulting the `google-developer-knowledge` MCP server or `search_web` tool for correct syntax — and re-run `preflight_audit.py` until it exits `0` with verdict `PASS`. Only then may Phase 4 GCP provisioning begin.
7. **Phase 4: Hermetic Verification (Full E2E Scope Only)**
   - Provision a pristine sandboxed test environment delegating initialization tasks to the **create-project** workflow and **gcp-provisioning** skill (which automatically disables org policies unless `--skip-org-policies` is passed).
   - Verify terminal execution reproducibility statefully by executing the unified **codelab-validation** stateful tester script (`python3 .agents/skills/codelab-validation/scripts/tester.py <lab.md> --project-id "<PROVISIONED_PROJECT_ID>" --artifact-dir <appDataDir>/brain/<conversation-id> --phase test`, where `<PROVISIONED_PROJECT_ID>` comes from `create_project.py`). **CRITICAL**: You MUST pass the active workspace artifact path down using the `--artifact-dir` command flag to route ephemeral status updates directly into live rendered HTML `task.md` preview buffers. Regardless of the Question 4 selection, ALWAYS run the tester with `--phase test` during verification so resources stay alive for quality review. Note: Cleanup steps will display `DEFERRED` on the task board during Phase 4 — this is expected behavior, as cleanup steps execute strictly in Phase 5.
   - Reflect runtime execution outputs inside the `task.md` output block containers.
   - **Sandbox vs Narrative Doctrine**: SYNTAX errors are remediated directly in the `.lab.md` narrative. ENVIRONMENT-specific remediations (org-policy overrides, quota/zone workarounds, `--no-address`, `--shielded-secure-boot`) go ONLY into `overlay.json` and MUST NEVER be baked into the published narrative.
8. **Phase 5: Quality Review, Clean-up Choice & Final Delivery (Full E2E Scope Only)**
   ...
   - **Sequencing Rule**: Cleanup NEVER runs before quality review, diagram generation, and any manual verification are complete.
   - **Clean-up Gate**: Only after quality review, diagram synthesis, and manual verification complete, process the Question 4 selection:
     - **Auto-delete**: Execute the explicit cleanup pass:
       `python3 .agents/skills/codelab-validation/scripts/tester.py <lab.md> --project-id "<PROVISIONED_PROJECT_ID>" --phase cleanup`
       (Using the same lab and state — the engine resumes, re-sources the `.env` cache, and runs only the `DEFERRED` steps, ensuring the cleanup section is genuinely tested). Confirm every cleanup step reports `DONE` on the task board, then proceed with project lifecycle handling.
     - **Retain**: Present an interactive `ask_question` modal to the user, offering them the choice to either **delete** all deployed GCP resources now (executing `--phase cleanup`) or **retain** them permanently for continuous experimentation.
   - Present validated Markdown artifacts to the user and transition to Phase 6.
9. **Phase 6: Retrospective & Continuous Improvement (Lessons Learned)**
...
1.  **Phase 0.5 Strategy Sign-Off**:
    - _Commander Action_: Draft `implementation_plan.md` in the active session brain directory. Show the plan clearly to the user in the chat and pause. Ask the user for explicit approval (e.g. "Approve Strategy Plan", "Request modifications") using the `ask_question` tool, gating further progress. Even if auto-approval hooks wake the agent up, it must block and wait for physical user interaction.
2.  **Phase 1 Parameter Scope Selection**:
    - _Commander Action_: Call the `ask_question` tool interactively at runtime to collect scope, persona, delivery format, and cleanup preferences, binding active variables and creating the unindented HTML `task.md` board.
3.  **Phase 2 Architectural Audit Loop**:
    - _Commander Action_: Programmatically spawn the `arch-critic` sub-agent using the `invoke_subagent` tool. The sub-agent audits `blueprint.md` in isolation and writes findings to `critic_response.json`. The Commander parses this JSON, auto-remediates low/medium severity issues in-place, and presents the finalized blueprint to the user via the `ask_question` modal for design approval.
4.  **Stateful Error Self-Healing (Dynamic Error Healer)**:
    - _Commander Action_: If a terminal command fails inside the persistent subshell (`tester.py`), the Commander intercepts the `stderr` directly. It attempts to self-heal the error in-line (e.g., fixing project IDs, missing flags, or API delays) and retries the command immediately in the active terminal session. On a step failure, edit the broken command in `.lab.md` and re-run `tester.py` WITHOUT `--fresh` against the SAME `--project-id` — the engine preserves `DONE` steps (e.g., 15-minute cluster builds), re-reads the edited commands, and re-executes only the failed step. `--fresh` is reserved for catastrophic state corruption (unparseable `.tester_state` JSON, unrecoverable state/infra divergence) and implies the old project's resources must be handled (cleanup pass or new project) — never use it as a retry mechanism. If unrecoverable, the Commander presents the failure directly to the user with clear remediation options, completely avoiding complex static mailbox queues.
