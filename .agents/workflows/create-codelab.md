...
7. **Phase 4: Hermetic Verification (Full E2E Scope Only)**
   ...
   - Verify terminal execution reproducibility statefully by executing the unified **codelab-validation** stateful tester script (`python3 .agents/skills/codelab-validation/scripts/tester.py <lab.md> --project-id "<PROVISIONED_PROJECT_ID>" --artifact-dir <appDataDir>/brain/<conversation-id>`, where `<PROVISIONED_PROJECT_ID>` comes from `create_project.py`). **CRITICAL**: You MUST pass the active workspace artifact path down using the `--artifact-dir` command flag to route ephemeral status updates directly into live rendered HTML `task.md` preview buffers. If the user selected "Retain active sandbox resources" in Question 4, you MUST append the `--phase test` (or deprecated `--skip-cleanup` alias) command flag to the tester execution call. If the user selected "Automatically delete", pass `--phase all` so that cleanup steps run during verification.
...
8. **Phase 5: Quality Review, Clean-up Choice & Final Delivery (Full E2E Scope Only)**
   ...
   - **Clean-up Choice Gate**: If the user selected "Automatically delete" in Question 4, confirm that resources have been fully destroyed by `--phase all` (or execute an explicit `--phase cleanup` pass: `python3 .agents/skills/codelab-validation/scripts/tester.py <lab.md> --project-id "<PROVISIONED_PROJECT_ID>" --phase cleanup`). If the user selected "Retain", present an interactive `ask_question` modal to the user, offering them the choice to either **delete** all deployed GCP resources now (executing `--phase cleanup`) or **retain** them permanently for continuous experimentation.
