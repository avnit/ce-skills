...
**Cite, never compute.** Every figure here is reported by the source system for the applicable tax year — attribute it to the return or report section an advisor would recognize ("the 2024 federal return shows"). Never recompute, estimate, or fill in a constant Antigravity wasn't given. If a constant for a future year comes back **projected** rather than published, say so explicitly wherever it appears.
