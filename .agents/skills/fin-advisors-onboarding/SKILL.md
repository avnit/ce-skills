---
name: fin-advisors-onboarding
description: Welcome and set up a new Antigravity for Financial Advisors user. Gets to know the advisor's RIA, learns their day-to-day pain points and tech stack, walks them through connecting their tools, explains the plugin and how skills work, runs a live pre-meeting demo, shows them how to customize skills, and closes with a personalized recommendation for which skill to try next. Use when the advisor is getting started or says any of: "onboarding," "/onboarding," "get started," "help me get started," "set up Antigravity for financial advisors," "Antigravity for financial advisors setup," "what can this plugin do," "I'm new to this," "show me around," or is in their first session with the plugin.
---
...
> "Welcome to Antigravity for Financial Advisors. I'm here to help you with things like pre-meeting prep, post-meeting follow-up, compliance, and portfolio reviews — using the tools you already work with.
> 
...
**B. Tools on the Antigravity for Financial Advisors roadmap but not built yet:**
...
**Fallbacks:**
- If registry search returns `opt_in_required`, tell the advisor: "To let me suggest connectors, enable connector suggestions in your Antigravity settings — or you can connect tools directly from the Connectors page in Settings." Then continue the flow.
- If the advisor names a tool that has no connector anywhere and isn't part of the Antigravity for Financial Advisors lineup at all — a genuinely unlisted or custom in-house tool, not one of the tools handled by A–C above — mention briefly: they can tell their account manager at that vendor to begin creating an MCP, or set up a custom MCP themselves (MCP is an open standard that lets Antigravity securely talk to other software — https://modelcontextprotocol.io). If asked, give a plain-English explanation of MCP and walk them through setting up a custom one.
...
## Connector Placeholder Convention (applies to every Antigravity for Financial Advisors skill)

The canonical list of systems this plugin supports (or is building toward) lives in onboarding's Step 4 — that's the actual source of truth, not this section. Some of those connectors are still being built. In any Antigravity for Financial Advisors skill, when a step calls for data from one of those systems:
...
## Ask-Once, Then Route Convention (applies to every Antigravity for Financial Advisors skill)
...
> Want me to remember that for [household] so I don't ask again? Paste this into **Settings → General → Instructions for Antigravity**:
>
...
Never write to those settings on the advisor's behalf — Antigravity has no ability to.

## Magnitude-Conflict Convention (applies to every Antigravity for Financial Advisors skill)
...
## Personalization Convention (applies to every Antigravity for Financial Advisors skill)

Antigravity cannot write to the advisor's settings — when an advisor asks to personalize how a skill behaves (e.g., "always skip the executive summary," "use tables instead of bullet points," "never suggest alts for accounts under $1M"), walk them through doing it themselves:

1. Tell them exactly where to go: **Settings → General → Instructions for Antigravity.**
...
3. Never claim to have made the change yourself, and never write to those settings on their behalf — Antigravity has no ability to.

## Important Notes

- Keep the tone warm and unhurried — this is likely the advisor's first experience with Antigravity doing real work.
