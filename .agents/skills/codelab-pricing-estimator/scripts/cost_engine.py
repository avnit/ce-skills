    ...
    try:
        ...
        if resource_type == "Compute Instance":
            if status in active_statuses:
                ...
                rate = billing_client.get_compute_price(machine_type, region)
                gpus = details.get("gpus", [])
                for gpu in gpus:
                    gpu_count = float(gpu.get("acceleratorCount", 1))
                    rate += 0.350 * gpu_count
                if details.get("scheduling", {}).get("provisioningModel") == "SPOT":
                    rate *= 0.30
                return rate
            return 0.0
            ...
            machine_type = details.get("machine_type", "e2-medium")
            return billing_client.get_gke_price(
                node_count, machine_type, region, status
            )

        elif resource_type == "Cloud SQL Database":
            if status in active_statuses:
                tier = details.get("tier", "db-f1-micro")
                rate = billing_client.get_sql_price(tier, region)
                if details.get("availability_type") == "REGIONAL" or details.get(
                    "is_ha"
                ):
                    rate *= 2.0
                return rate
            ...
            return billing_client.get_vertex_price(
                subtype, machine_type, node_count, region
            )
...
def format_html_table(resources):
    ...
    if not resources:
        ...
        lines.append(
            '<td colspan="6" style="padding: 20px 24px; text-align: center; color: #5f6368;">No billable active resources discovered.</td>'
        )
        lines.append("</tr>")
    else:
        for idx, r in enumerate(resources):
            ...
            lines.append(
                f'<tr style="border-bottom: 1px solid #e8eaed; background-color: {bg_color};">'
            )
            lines.append(
                f'<td style="padding: 14px 24px; vertical-align: top; font-weight: 600; color: #3c4043;">`{r.get("name", "N/A")}`</td>'
            )
            lines.append(
                f'<td style="padding: 14px 12px; vertical-align: top; color: #5f6368;">{r.get("type", "N/A")}</td>'
            )
            ...
            status_style = (
                ...
                else "background: #f1f3f4; color: #5f6368;"
            )
            lines.append(
                f'<td style="padding: 14px 12px; vertical-align: top;"><span style="{status_style} padding: 4px 10px; border-radius: 12px; font-weight: 600; font-size: 11px; display: inline-block;">{status}</span></td>'
            )

            lines.append(
                f'<td style="padding: 14px 12px; vertical-align: top; color: #5f6368;">{r.get("config", "N/A")}</td>'
            )
            lines.append(
                f'<td style="padding: 14px 12px; vertical-align: top; color: #5f6368;">{r.get("location", "N/A")}</td>'
            )
            lines.append(
                f'<td style="padding: 14px 24px; vertical-align: top; font-weight: 600; color: #1a73e8;">${r.get("hourly_cost", 0.0):.4f}</td>'
            )
            lines.append("</tr>")
