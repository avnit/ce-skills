...
def audit_project_resources(project_id):
    ...
    if not success or not stdout:
        print(
            "⚠️ Cloud Asset Inventory query timed out or unavailable. Falling back to direct native service queries across all supported categories..."
        )
        assets = []
        ...
        try:
            ...
            asset_types = (
                [a.get("assetType", "") for a in assets]
                if assets
                else default_asset_types
            )
            print(
                f"📋 Discovered {len(assets)} total resources across {len(set(asset_types))} unique asset types."
            )
        except Exception as e:
            print(
                f"⚠️ Error parsing assets JSON: {e}. Falling back to direct native service queries..."
            )
            assets = []
            asset_types = default_asset_types
    ...
    # SWP Gateways
    ok, out, _ = billing_client.run_command(
        [
            "gcloud",
            "network-services",
            "gateways",
        ...
        try:
            for item in json.loads(out):
                ...
                add_resource(
                    ...
                    region,
                    {"status": "ACTIVE", "subtype": "swp_gateway", "count": 1},
                )
        except Exception:
            pass
    swp_assets = [
        a
        for a in assets
        if "networkservices.googleapis.com/Gateway" in a.get("assetType", "")
    ]
    for a in swp_assets:
        ...
        try:
            for item in json.loads(out):
                ...
                add_resource(
                    ...
                    "Networking / Security",
                    "ACTIVE",
                    "Cloud Firewall Plus Endpoint",
                    location,
                    {"status": "ACTIVE", "subtype": "firewall_endpoint", "count": 1},
                )
        except Exception:
            pass
    fw_assets = [
        a
        for a in assets
        if "networksecurity.googleapis.com/FirewallEndpoint" in a.get("assetType", "")
    ]
    for a in fw_assets:
        ...
        try:
            for item in json.loads(out):
                ...
                add_resource(
                    ...
                    "Networking / Security",
                    "ACTIVE",
                    f"HA VPN Gateway ({len(tunnels)} Interfaces)",
                    region,
                    {
                        "status": "ACTIVE",
                        "subtype": "vpn_gateway",
                        "count": max(1, len(tunnels)),
                    },
                )
        except Exception:
            pass
    vpn_assets = [
        a
        for a in assets
        if "compute.googleapis.com/VpnGateway" in a.get("assetType", "")
    ]
    for a in vpn_assets:
        ...
        if name not in handled_resource_names:
            ...
            add_resource(
                ...
                "Networking / Security",
                "ACTIVE",
                "HA VPN Gateway (2 Tunnels Est.)",
                loc,
                {"status": "ACTIVE", "subtype": "vpn_gateway", "count": 2},
            )
            ...
            try:
                for item in json.loads(out):
                    ...
                    if nats:
                        ...
                        region_url = item.get("region", "")
                        region = (
                            region_url.split("/")[-1] if region_url else "us-central1"
                        )
                        add_resource(
                            f"{name}-nat",
                            "Networking / Security",
                            "ACTIVE",
                            f"Cloud NAT ({len(nats)} gateways)",
                            region,
                            {
                                "status": "ACTIVE",
                                "subtype": "nat_gateway",
        ...
        try:
            ...
            if items:
                add_resource(
                    ...
                    "Networking / Security",
                    "ACTIVE",
                    f"{len(items)} Forwarding Rules",
                    "global",
                    {
                        "status": "ACTIVE",
                        "subtype": "forwarding_rule",
                        "count": len(items),
                    },
                )
        except Exception:
    ...
    if "load-balancer-rules" not in handled_resource_names:
        fwd_assets = [
            a
            for a in assets
            if "compute.googleapis.com/ForwardingRule" in a.get("assetType", "")
        ]
        if fwd_assets:
            add_resource(
                "load-balancer-rules",
                "Networking / Security",
                "ACTIVE",
                f"{len(fwd_assets)} Forwarding Rules",
                "global",
                {
                    "status": "ACTIVE",
                    "subtype": "forwarding_rule",
                    "count": len(fwd_assets),
                },
            )
    ...
    if any("aiplatform.googleapis.com" in t for t in asset_types):
        ...
        if ok and out:
            try:
                for item in json.loads(out):
                    name = item.get(
                        "displayName", item.get("name", "unnamed-ep").split("/")[-1]
                    )
                    ...
                    for dm in deployed_models:
                        machine_spec = dm.get("dedicatedResources", {}).get(
                            "machineSpec", {}
                        )
                        mtype = machine_spec.get("machineType", "n1-standard-4")
                        rep_count = int(
                            dm.get("dedicatedResources", {}).get("minReplicaCount", 1)
                        )
                    ...
                    if total_nodes > 0:
                        ...
                        add_resource(
                            ...
                            "Vertex AI Endpoint",
                            "ACTIVE",
                            f"Dedicated VM ({cfg_str})",
                            region,
                            {
                                "status": "ACTIVE",
                                "subtype": "model_endpoint",
                                "machine_type": mtype,
        ...
        mdl_assets = [
            a
            for a in assets
            if "aiplatform.googleapis.com/Endpoint" in a.get("assetType", "")
        ]
        for a in mdl_assets:
            ...
            if asset_name not in handled_resource_names:
                ...
                add_resource(
                    ...
                    "Vertex AI Endpoint",
                    "ACTIVE",
                    "Dedicated Model Endpoint VM",
                    loc,
                    {"status": "ACTIVE", "subtype": "model_endpoint", "node_count": 1},
                )
        ...
        if ok and out:
            try:
                for item in json.loads(out):
                    name = item.get(
                        "displayName", item.get("name", "unnamed-idx").split("/")[-1]
                    )
                    parts = item.get("name", "").split("/")
                    region = parts[3] if len(parts) > 3 else "us-central1"
                    add_resource(
                        ...
                        "Vertex AI Endpoint",
                        "ACTIVE",
                        "Provisioned Vector Search Node",
                        region,
                        {
                            "status": "ACTIVE",
                            "subtype": "index_endpoint",
                            "node_count": 1,
                        },
                    )
            except Exception:
        ...
        idx_assets = [
            a
            for a in assets
            if "aiplatform.googleapis.com/IndexEndpoint" in a.get("assetType", "")
        ]
        for a in idx_assets:
            ...
            if asset_name not in handled_resource_names:
                ...
                add_resource(
                    ...
                    "Provisioned Vector Search Node",
                    loc,
                    {"status": "ACTIVE", "subtype": "index_endpoint", "node_count": 1},
                )
            ...
            try:
                for item in json.loads(out):
                    ...
                    if min_scale > 0 or cpu_throttling == "false":
                        instances_to_bill = max(
                            min_scale, 1 if cpu_throttling == "false" else 0
                        )
                        ...
                        vcpu = (
                            float(cpu_str.replace("m", "")) / 1000.0
                            if "m" in cpu_str
                            else float(cpu_str)
                        )
                        mem_gb = (
                            float(mem_str.replace("Mi", "")) / 1024.0
                            if "Mi" in mem_str
                            else (
                                float(mem_str.replace("Gi", ""))
                                if "Gi" in mem_str
                                else 0.5
                            )
                        )
                        ...
                        add_resource(
                            ...
                            raw_data={
                                "status": "ALLOCATED",
                                "min_instances": instances_to_bill,
                                "vcpu": vcpu,
                                "memory_gb": mem_gb,
                            },
                        )
    ...
    for a in assets:
        ...
        if not any(a_type.startswith(p) for p in known_handled_prefixes):
            if any(
                ...
                loc = a.get("location", "global")
                service_name = (
                    a_type.split("/")[0].replace(".googleapis.com", "").upper()
                )
                add_resource(
                    name,
                    "Universal Provisioned Resource",
                    "PROVISIONED",
                    f"Service: {service_name}",
                    loc,
                    {"status": "PROVISIONED", "asset_type": a_type},
                )
