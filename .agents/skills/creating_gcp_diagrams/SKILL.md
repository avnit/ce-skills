...
*   **Mermaid Syntax Errors**: To prevent syntax errors inside diagrams, always double-quote node labels containing special characters like parentheses or brackets. E.g. use `id["Label (Extra Info)"]` instead of `id[Label (Extra Info)]`.
*   **Avoiding HTML Tags**: Do not use raw HTML tags within node labels as they can break rendering pipelines.
*   **Model Hallucinations in Images**: If the image generation model renders garbled text inside cards, refine your text prompts to be simpler, or request SVGs with standard typography elements.
*   **Image Storage Location**: Ensure that generated diagrams are copied into the specific example/lab subdirectory, rather than being left in the root `/tmp` or workspace folders.
