...
3. **MCP Configuration Injection**: Updates `.gemini/mcp_config.json` to inject `X-goog-user-project` headers for `google-developer-knowledge` and ensures the `workspace` MCP server binary is registered.
