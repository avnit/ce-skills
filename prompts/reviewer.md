...
-   **Header**:
    -   `id` is lowercase, hyphen-separated, URL-safe.
    -   `summary` is one line, under 200 characters.
    -   `keywords` includes descriptive tags for product and category with NO spaces after commas.
    -   `layout: paginated` is set.
    -   No blank lines inside the `---` delimiters.
    -   Exactly one blank line between closing `---` and `# Title`.
-   **Structure**:
    -   First step is "Introduction".
    -   Introduction includes "What you'll do" and "What you'll need".
