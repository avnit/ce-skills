...
1. **GCP Config Check**: Validates that `/Users/shacharb/Downloads/ce-scale/gcp_config.txt` exists and contains non-empty parameters for `folder_id`, `billing_account`, and `cloudtop_host`.
2. **MCP Servers Check**: Parses `/Users/shacharb/Downloads/ce-scale/.gemini/mcp_config.json` and tests connection to each configured HTTP MCP server using a POST JSON-RPC payload.
