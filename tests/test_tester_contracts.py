...
class TestTesterContracts(unittest.TestCase):
    ...
    def test_exit_semantics_passing_and_failing(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_str:
            ...
            md_file = tmp_dir / "lab_fail.md"
            md_file.write_text(md_content_fail, encoding="utf-8")

            t_fail = tester.StatefulCodelabTester(str(md_file))
            with patch.object(tester, "get_active_project", return_value="mock-project"):
                success = t_fail.run()
            ...
            progress_file = tmp_dir / ".tester_state" / "progress.json"
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_str:
            tmp_dir = Path(tmp_dir_str)
            md_file = tmp_dir / "lab_pass.md"
            md_file.write_text(md_content_pass, encoding="utf-8")

            t_pass = tester.StatefulCodelabTester(str(md_file))
            with patch.object(tester, "get_active_project", return_value="mock-project"):
                success = t_pass.run()
    ...
    def test_subshell_runner_exit_code_protocol(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_str:
            runner = tester.SubshellRunner(cwd=tmp_dir_str)
            try:
    ...
    def test_hash_cache_skips_and_invalidates_on_edit(self):
        """2. Hash-cache: second run skips executed command; edit invalidates and re-runs."""
        md_content_1 = (
            "## Step 1: Cache Step\n"
            "```bash\n"
            "echo cache-me\n"
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_str:
            ...
            # Reset step state to PENDING to allow tester to evaluate edited step
            step_1_file = tmp_dir / ".tester_state" / "step-001.json"
            if step_1_file.exists():
    ...
    def test_bug_json_contract_v1_schema_and_locations(self):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir_str:
            ...
            with patch("os.path.expanduser", side_effect=mock_expanduser), \
                 patch.object(tester.subprocess, "Popen") as mock_popen:
                ...
                self.assertIn("bug_to_lesson_processor.py", cmd_args[1])

            local_bugs_dir = tmp_dir / "bugs"
            local_bug_files = list(local_bugs_dir.glob("bug_*.json"))
            self.assertEqual(len(local_bug_files), 1)

            central_bugs_dir = fake_home / ".gemini" / "jetski" / "bugs"
